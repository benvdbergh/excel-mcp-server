...
class ComWorkbookService:
    ...
    def _query_table_com(
        ...
        try:
            payload = _com_query_table_rows_from_list_object(
                lo, str(lo.Name), columns, where, limit, offset
            )
            return json.dumps(payload, indent=2, default=str)
        except DataError as e:
            return f"Error: {str(e)}"
    ...
    def _apply_table_view_com(
        ...
        wb_com, err = ComWorkbookService._get_open_workbook_com(
        ...
        try:
            ...
            if mode_norm == VIEW_MODE_SNAPSHOT:
                return ComWorkbookService._apply_snapshot_view_com(
            ...
            if target_kind == "table":
                return ComWorkbookService._apply_listobject_view_com(
                    wb_com, view_spec, mode, view_applicability
                )
            if target_kind == "region":
                return ComWorkbookService._apply_range_view_com(
                    wb_com, view_spec, mode, view_applicability
                )
    ...
    def _clear_table_view_com(
        ...
        try:
            ...
            if token["kind"] == RESTORE_KIND_LISTOBJECT:
                return ComWorkbookService._clear_listobject_view_com(wb_com, token)
            if token["kind"] == RESTORE_KIND_RANGE:
                return ComWorkbookService._clear_range_view_com(wb_com, token)
            return ComWorkbookService._clear_snapshot_view_com(wb_com, token)
        except DataError as e:
            return f"Error: {str(e)}"
        except Exception as e:
            return f"Error: {e}"
